  ...
  useEffect(() => {
    ...
    const handleCredentialResponse = async (response: any) => {
      try {
        ...
        // Decode the credential (optional, but we need it for email/name if backend doesn't verify)
        // Since we want the backend to verify, we just pass the credential (idToken)
        const idToken = response.credential;
        
        // We'll let the backend decode it, but for UI we can decode locally too
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white dark:bg-slate-800 text-slate-500">Atau lanjut dengan</span>
              </div>
            </div>

            <div id="google-login-button" className="w-full min-h-[44px]"></div>
          </form>
